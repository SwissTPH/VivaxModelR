test_that("test calculation of lambda on data, with RCD", {
  ...
  mydata=data.frame(incidence=c(127.4453, 91.19589,23.48002,69.63335, 56.6991,43.63537),
                    prop_import=c(0,0,0, 0.2072365 ,0.2548594,0.3318546),
                    ...
                    nu=c(5,5,5,5,5,5), tau=c(5,5,5,5,5,5),eta=c(1,0.9,1,1,0.9,1),kappa=c(1,1,1,0.8,0.8,0.8))%>%
    dplyr::mutate(h=incidence_year2day(incidence))
...
test_that("test calculation of lambda on data, with RCD and delays", {
  ...
  mydata=data.frame(incidence=c(100.6487    , 63.37615,15.90299   ,86.72849  , 67.98483 ,49.51637 ),
                    prop_import=c(0,0,0, 0.1657857   ,0.2119406  ,0.2918947 ),
  ...
  mydata_ref=data.frame(incidence=c(140.7197   , 84.83822,20.52478   ,93.7199   , 71.3841 ,50.90783 ),
                        prop_import=c(0,0,0, 0.1531888     ,0.2016693  ,0.2837854 ),
                        alpha=c(0.1,0.1,0.1,0.4,0.4,0.4),
                        beta=c(1,1,1,0.7,0.7,0.7),
                        rho=c(1,1,1,0.4,0.4,0.4),
                        omega=c(1,1,1,0.8,0.8,0.8),
...
test_that("test simulation of future scenarios, with rcd", {
  ...
  simul_rcd_true=data.frame(time=rep(1825, 3),
                            ...
                            h=c(0.03885228 ,0.19372276    ,0.64698395    ), hr=c(0.02615755 ,0.05721143,0.12757704  ),
                            hl=c(0.03657982  ,0.18197269   ,0.62126821   ),
                            hh=c(0.215846  ,0.682824 ,3.827577   ),hhl=c(0.2032212  ,0.5924389 ,3.5061300   ),
                            I=c(0.02986417,0.08081288 ,0.52511039  ), p=c(0.05848969    ,0.06065405   , 0.03974711    ),
                            incidence=c(38.85228, 193.72276   ,646.98395    ), id=c(1,2,3))   #it's ok if we are not stable for id=1 because lambda, delta and omega do not correspond to an equilibrium
...
test_that("test simulation of future scenarios, with rcd, with delays", {
  ...
  simul_rcd_true=data.frame(time=rep(1825, 3),
                            ...
                            hl=c(0.04480642   ,0.19145823     ,0.60395098 ),
                            hh=c(0.2614521   ,0.7316038   ,3.8148497   ),
                            hhl=c(0.2489246    ,0.6420784    ,3.4986731    ),
                            I=c(0.03799354 ,0.08960370  ,0.53289624   ), p=c(0.04791506      ,0.05730428     , 0.04019758     ),
                            incidence=c(47.06137    , 203.09653,629.24511 ), id=c(1,2,3)) #it's ok if we are not stable for id=1 because lambda, delta and omega do not correspond to an equilibrium
  ...
  simul_rcd_ref_true=data.frame(time=rep(1825, 3),
                            ...
                            hl=c(0.04480642   ,0.20908787       ,0.60182936       ),
                            hh=c(0.2614521   ,0.7947005    ,3.7990455    ),
                            hhl=c(0.2489246    ,0.7063323     ,3.4889343  ),
                            I=c(0.03799354 ,0.10143549   ,0.54185704    ), p=c(0.04791506        ,0.05208130     , 0.03959046      ),
                            incidence=c(47.06137    , 220.57574  ,626.63826            ), id=c(1,2,3))
